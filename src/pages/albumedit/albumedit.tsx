...
const AlbumEdit: React.FC = () => {
    ...
    const albumData = useRef<AlbumInfo>(null);
    const formData = useRef<AlbumFormData>(null);
    ...
    async function fetchAlbumData(): Promise<void> {
        try {
            const docRef = await getDoc(doc(db, 'albums', id));
            albumData.current = docRef.data() as AlbumInfo;
            albumData.current.artwork = `${prereq}${id}/artwork.${albumData.current.ext}`;
            formData.current = {
                name: albumData.current.name,
                description: albumData.current.description,
                ext: albumData.current.ext,
    ...
    async function onSave(): Promise<void> {
        try {
            ...
            if (validation) {
                ...
                await editAlbumData(id, album, songs);
